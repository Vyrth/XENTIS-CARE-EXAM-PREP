/**
 * Notebook Summary - POST /api/ai/notebook-summary
 * Converts messy notes or highlighted content into clean, board-focused summaries.
...
import { runNotebookSummary } from "@/lib/ai/notebook-summary";
import { retrieveForNotebookSummary } from "@/lib/ai/retrieval/retrieve-for-action";
import {
...
import type { ExamTrack, SummaryMode } from "@/lib/ai/notebook-summary/types";
...
function validateRequest(
  ...
  const trimmed = body.noteText.trim();
  if (!trimmed) {
    return { valid: false, error: "noteText cannot be empty", status: 400 };
  }

  if (trimmed.length > MAX_NOTE_TEXT) {
    return {
      valid: false,
      error: `noteText exceeds max length (${MAX_NOTE_TEXT} chars)`,
      status: 400,
    };
  }
  ...
  if (
    body.summaryMode &&
    !VALID_MODES.includes(body.summaryMode as SummaryMode)
  ) {
    return {
      valid: false,
      error: `summaryMode must be one of: ${VALID_MODES.join(", ")}`,
      status: 400,
    };
  }
...
export async function POST(request: Request) {
  ...
  const [retrieveResult, analytics] = await Promise.all([
    retrieveForNotebookSummary({
      query: data.noteText!.trim(),
      examTrack,
  ...
  const result = await runNotebookSummary(
    {
      ...
      sourceType: data.sourceType,
      sourceId: data.sourceId,
      summaryMode: (data.summaryMode as SummaryMode) ?? "clean_summary",
    },
    {
      userId: user?.id,
      retrievedContext,
