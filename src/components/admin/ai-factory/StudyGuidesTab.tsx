...
import { StudyGuidePreview, type StudyGuideDraft } from "./preview";
import {
...
export interface StudyGuidesTabProps {
...
export function StudyGuidesTab({ config, data, onConfigChange, pendingGeneratePreset, onGenerateTriggered }: StudyGuidesTabProps) {
  const router = useRouter();
  const [generating, setGenerating] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [fieldErrors, setFieldErrors] = useState<FieldErrors>({});
  const [preview, setPreview] = useState<StudyGuideDraft | null>(null);
  const [auditId, setAuditId] = useState<string | null>(null);

  const mode = config.studyGuideMode ?? "full";
  ...
  const handleGeneratePreview = async () => {
    ...
    const validation = validateGenerationConfig(config, "study_guide", data.tracks);
    ...
    setGenerating(true);
    try {
      const result = await generateStudyGuideDraft(fullConfig, mode);
      if (result.success && result.draft) {
        setPreview(result.draft as StudyGuideDraft);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const result = await saveStudyGuideDraft(fullConfig, preview, auditId ?? undefined);
      if (result.success && result.contentId) {
        router.push(`/admin/study-guides/${result.contentId}`);
      } else {
  ...
  return (
    <Card>
      <h2 className="font-semibold text-slate-900 dark:text-white mb-4">Generate Study Guide</h2>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
        Generate board-focused study guides. Preview, edit sections, and save only when ready. Nothing is persisted until you click Save.
      </p>
      ...
      <GenerationConfigPanel
        ...
        showStudyGuideMode
        showBoardFocus
        showSectionCount={mode === "section_pack"}
        showObjective
        showDifficulty
      />
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        {resolvedConfig && (
          <a
            href={`/admin/study-guides/new?trackId=${resolvedConfig.trackId}`}
